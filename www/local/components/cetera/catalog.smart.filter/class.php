...
class FacetMod extends \Bitrix\Iblock\PropertyIndex\Facet {
    public function query(array $filter, array $facetTypes = array(), $facetId = 0)
    {
        ...
        if ($filter)
        {
            ...
            //доп.фильтр
            if(is_array($GLOBALS["sFS"]))
                $filter=array_merge($filter,$GLOBALS["sFS"]);

            unset($filter['OFFERS']);
            if (is_array($GLOBALS["sFS"]["OFFERS"]))
                $filter["ID"] = CIBlockElement::SubQuery('PROPERTY_CML2_LINK', $GLOBALS["sFS"]["OFFERS"]);
...
class CBitrixCatalogSmartFilterMod extends CBitrixComponent
{
	public $IBLOCK_ID = 0;
