        ...
        foreach ($arResult['ORDERS'] as $key => $order)
        {
            ...
            <div class="callout">
                ...
                <h5 class="text-dark-gray">
                    <?=Loc::getMessage('SPOL_TPL_PAYMENT')?>
                </h5>
                ...
                foreach ($order['PAYMENT'] as $payment)
                {
                    ...
                    <div class="grid-x grid-padding-x small-up-1 medium-up-2 large-up-12">
                    <div class="cell small-12 medium-8">
                <?}?>
                    <div class="margin-bottom-6 sale-order-list-inner-row">
                        <?
                        if ($showDelimeter)
                        {
                            ?>
                        ...
                        <div class="sale-order-list-inner-row-body">
                            <div class="col-md-9 col-sm-8 col-xs-12 sale-order-list-payment">
                                ...
                                {
                                    ?>
                                    <span class="label"><?=Loc::getMessage('SPOL_TPL_NOTPAID')?></span>
                                    <?
                                }
                                ?>
                                <br>
                ...
                {
                    ?>
                    <h5 class="text-dark-gray"><?=Loc::getMessage('SPOL_TPL_DELIVERY')?></h5>
                    <?
                }
                $showDelimeter = false;
                foreach ($order['SHIPMENT'] as $shipment)
                {
                    ...
                    <div class="margin-bottom-6">
                        ...
                        {
                            ?>
                            <span class="label success"><?=Loc::getMessage('SPOL_TPL_LOADED');?></span>
                            <?
                        }
                        else
                        {
                            ?>
                            <span class="label"><?=Loc::getMessage('SPOL_TPL_NOTLOADED');?></span>
                            <?
                        }
                        ?>

                        <br>
                        <?=Loc::getMessage('SPOL_ORDER_SHIPMENT_STATUS');?>:<span class="label light-gray"><?=htmlspecialcharsbx($shipment['DELIVERY_STATUS_NAME'])?></span>
                        <br>
                ...
                <div class="grid-x grid-padding-x">
                    <div class="cell small-12 medium-4">
                        <a class="" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_DETAIL"])?>"><?=Loc::getMessage('SPOL_TPL_MORE_ON_ORDER')?></a>
                    </div>
                    <div class="cell small-12 medium-8 medium-text-right">
                        <i class="fa fa-undo"></i><a class="" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_COPY"])?>"><?=Loc::getMessage('SPOL_TPL_REPEAT_ORDER')?></a>
                        <a class="text-dark-gray" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_CANCEL"])?>"><?=Loc::getMessage('SPOL_TPL_CANCEL_ORDER')?></a>
                    </div>
                </div>
